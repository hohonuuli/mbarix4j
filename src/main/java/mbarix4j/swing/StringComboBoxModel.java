...
public class StringComboBoxModel implements MutableComboBoxModel, ListModel {
    ...
    private void checkItemType(Object item) throws ClassCastException {
        if (!(item instanceof String)) {
            throw new ClassCastException(
                    "StringComboBoxModel only support String elements.");
        }
    }
     ...
     * @param index_1
     */
    protected void fireContentsChanged(Object source, int index_0,
            int index_1) {
            ...
            if (listeners[i] == ListDataListener.class) {
                if (event == null) {
                    event = new ListDataEvent(source,
                            ListDataEvent.CONTENTS_CHANGED, index_0, index_1);
                }
    ...
    protected void fireIntervalAdded(Object source, int index_0, int index_1) {
        ...
        for (int i = listeners.length - 2; 0 <= i; i -= 2) {
            if (listeners[i] == ListDataListener.class) {
                if (event == null) {
                    event = new ListDataEvent(source,
                            ListDataEvent.INTERVAL_ADDED, index_0, index_1);
                }
     ...
     * @param index_1
     */
    protected void fireIntervalRemoved(Object source, int index_0,
            int index_1) {
        Object[] listeners = listenerList.getListenerList();
        ListDataEvent event = null;
        for (int i = listeners.length - 2; 0 <= i; i -= 2) {
            if (listeners[i] == ListDataListener.class) {
                if (event == null) {
                    event = new ListDataEvent(source,
                            ListDataEvent.INTERVAL_REMOVED, index_0, index_1);
                }
    ...
    public int getMaxLength() {
        if (maxItemLength == 0) {
            for (int i = 0; i < items.size(); i++) {
                maxItemLength = Math.max(maxItemLength,
                        ((String) items.get(i)).length());
            }
        }
    ...
    public void removeElementAt(int index) {
        if (getElementAt(index) == selectedItem) {
            if (index == 0) {
                setSelectedItem(
                        (items.size() == 1) ? null : getElementAt(index + 1));
            } else {
