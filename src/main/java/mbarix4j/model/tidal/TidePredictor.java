...
public class TidePredictor {
	...
	public TidePredictor() {
		...
		logger.debug("Default start time is " + startDateTime);
		aBriefMomentInTime.add(Calendar.HOUR, 24);
		endDateTime = aBriefMomentInTime.getTime();
		logger.debug("Default end time is " + endDateTime);
		...
		logger
        .debug("Tried to get input stream from props file, how did I do? propStream="
               + propStream);
		...
		try {
			...
			logger.error("Throwable caught trying to read properties: "
                         + t.getMessage(), false);
		}
		...
		logger.debug("Tide location URL string read from properties is "
                     + this.tideLocationListUrlString);
	...
	public HashMap<String, String> getLikeTideLocationsFromService(
                                                                   String likeString) {
		logger.debug("Will look for similar locations at: "
                     + this.tideLocationListUrlString);
		...
		try {
			logger
            .debug("Going to try and find the similar locations from the URL "
                   + tideLocationListUrlString);
		...
		try {
			...
			logger.error("IOException trying to read location list from URL: "
                         + e.getMessage());
		}
		...
		try {
			...
			logger
            .error("IOException caught reading from the location list URL: "
                   + e.getMessage());
		}
		...
		if ((tideLocation == null) || (tideLocation.equals(""))
			...
			if (!this.isDateRangeCovered(startDateTime, endDateTime)) {
				...
				try {
					...
					logger
                    .error("Throwable caught trying to read tide data and events from URL: "
                           + t.getMessage());
				}
				// Set the flag to indicate we are buys
				this.busyRetrievingData = false;
	...
	private HashMap<Date, String> readTideEventsFromService() {
		...
		try {
			...
			logger
            .error("MalforedURLException trying to create URL to read tide events"
                   + ex.getMessage());
		}
		...
		try {
			...
			logger.error("IOException caught trying to open "
                         + "buffered reader to URL for tide events: "
                         + e.getMessage());
		}
