/*
 * @(#)TimeUtilities.java   2011.12.30 at 09:19:19 PST
 *
...
public class TimeUtilities {
    ...
    private static final DateTimeFormatter compactTimeFormatter = DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmssX");

    public static Optional<Instant> parseIso8601(String s) {
        Supplier<Optional<Instant>> f0 = () -> {
            try {
                return Optional.of(Instant.from(compactTimeFormatter.parse(s)));
            } catch (Exception e) {
                return Optional.empty();
            }};
        Supplier<Optional<Instant>> f1 = () -> {
            try {
                return Optional.of(Instant.from(timeFormatter.parse(s)));
            } catch (Exception e) {
                return Optional.empty();
            }};
        Supplier<Optional<Instant>> f2 = () -> {
            try {
    ...
    public static Date dayOfYearToDate(double dayOfYear, double year) {
        ...
        GmtCalendar gc = new GmtCalendar((int) year, month - 1, (int) Math.floor(day), (int) Math.floor(hour),
